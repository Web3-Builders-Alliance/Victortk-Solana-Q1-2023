...
use farm_program::{self, CultivarMeta, Farm, TreesMeta};
use tree_program::{self, Cultivar, Tree};

declare_id!("BdyXR2oKTEyeV8FRmRA2F7dxHjtkWeBtMEHqZPSQVuVd");
...
pub struct InitializeFruitMarket<'info> {
    ...
    #[account(seeds=[b"tree",trees_meta.key().as_ref(),farmer.key().as_ref(),tree.cultivar_name.as_bytes().as_ref(),tree.created_date.as_bytes()], bump, seeds::program=tree_program )]
